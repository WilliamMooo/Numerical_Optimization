...
function [alpha_star,iter] = lsearch_SWolfe(xk,fk,gk,dk,OPTIONS,lsopts,alpha_max,lsmax,A,b)
...
for i=1:lsmax
    ...
    [fnew,gnew] = objfun(xnew,A,b);
    
    if (fnew> fk + c1*alpha*cratio) || (i>1 && fnew>=fold)
        
        alpha_star = zoom(alpha_old,alpha,fold,xk,fk,dk,cratio,lsopts,A,b);
    ...
    elseif cratio_new>=0
        ...
        alpha_star = zoom(alpha,alpha_old,fnew,xk,fk,dk,cratio,lsopts,A,b);
...
%% ********************* zoom function *****************************
%%
function alpha_star = zoom(lalpha,halpha,flow,xk,fk,dk,cratio,lsopts,A,b)
